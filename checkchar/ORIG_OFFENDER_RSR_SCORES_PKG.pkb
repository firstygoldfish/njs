...
--
/*
	OFFENDER_RSR_SCORE_PK
	OFFENDER_PK (from the offender record)
	PNC (from the offender record)
	FAMILY_NAME (from the offender record)
	FORENAME_1 (from the offender record)
	DATE_OF_BIRTH (from the offender record)
	CMS_PROB_NUMBER (from the offender record)
	GENDER_CAT and GENDER_ELM (from the offender record)
	INITIATION_DATE (system date and time)
	RSR_SCORE_USER (Username)
	RSR_SCORE_USER_NAME (forename1 and surname of the user)
	RSR_STATUS (set to OPEN)
	RSR_ALGORITHM_VERSION (set to the number on the global profile parameter)
*/
    lv_row := NULL;
...
If the OFFENDER.CUSTODY_IND = 'Y'
	Set OFFENDER_RSR_SCORES.PRISON_IND = 'C'
Otherwise
	If OFFENDER.REMAND_IND = 'Y'
		Set OFFENDER_RSR_SCORES.PRISON_IND = 'R'
	Otherwise
		Leave OFFENDER_RSR_SCORES.PRISON_IND blank (offender is not in prison)
	End
	Set OFFENDER_RSR_SCORES.PREV_OSP_C_RISK_RED_TEXT = NULL (may have cloned through so clear it out)
	Set OFFENDER_RSR_SCORES. S1_43_LAST_OFFENCE_DATE = NULL (may have cloned through so clear it out)
End
